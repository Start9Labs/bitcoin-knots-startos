...
>
> Enforcement-model precision (matters for development): the official Knots release binaries this package ships are built with `RDTS_CONSENT=RUNTIME_WARN` (upstream `contrib/guix/libexec/build.sh`), so the binary **enforces RDTS on mainnet from its first start regardless of `consensusrules`** — that option only recorded user _consent_ and silenced the binary's periodic consent warning; it was never an enforcement toggle. The opt-in is a package-level gate now (`rdtsAcknowledged`), so `consensusrules=rdts` is **pinned as an enforced field** — written on every merge alongside `rpccookiefile` and `deprecatedrpc` — for one reason only: without it the binary warns on every start. Do not read it as consent, and do not expose it as a setting. Because it is enforced, `bitcoinConfFile.merge` cannot remove it — every merge re-stamps it — so the Core-facing `down` migrations strip the line from `bitcoin.conf` directly on the way out, which is the only reason that code touches the file by path. Never derive "is this node enforcing" from `bitcoin.conf`; the chain-recovery oneshot asks the node itself (`getdeploymentinfo` — the `reduced_data` deployment is present exactly when enforcement is enabled).
