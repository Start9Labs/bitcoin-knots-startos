...
export const activateRDTS = sdk.Action.withInput(
  ...
  async ({ effects, input }) => {
    ...
    // regardless of this option (see forkRecovery.ts) — re-validation after
    // an enforcement-regime change is driven by the chain-recovery oneshot
    // (store marker + migration flags), not by this acknowledgment.
